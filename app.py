from flask import Flask, jsonify, request # request add kiya key check karne ke liye
import aiohttp
import asyncio
import json
...
# --- Baki saare functions (load_tokens, get_url, parse_protobuf_response, visit, send_until_10000_success) bilkul same rahenge ---
...
async def send_until_10000_success(tokens, uid, server_name, target_success=10000):
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        while total_success < target_success:
            ...
            print(f"Batch sent: {batch_size}, Success: {total_success}")
    return total_success, total_sent, player_info

# --- Modified Route with Key Support ---

@app.route('/<string:server>/<int:uid>', methods=['GET'])
def send_visits(server, uid):
    # URL query se 'key' nikalna (e.g., ?key=Flash)
    ...
    print(f"🚀 Sending visits to UID: {uid} using key: {user_key}")
